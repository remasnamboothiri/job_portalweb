...
#         # FIXED: HANDLE POST REQUEST - SPEECH RESPONSES
#         if request.method == "POST":
#             try:
...
#                 logger.info(f"Received user input for interview {interview_uuid}: '{user_text}' (length: {len(user_text)})")
                
#             except Exception as parse_error:
#                 logger.error(f"Error parsing request data for interview {interview_uuid}: {parse_error}")
#                 user_text = request.POST.get("text", "").strip()
    
#             # FIXED: Validate input - allow any meaningful speech
#             if not user_text or len(user_text.strip()) < 2:
#                 logger.warning(f"Empty or too short input received for interview {interview_uuid}: '{user_text}'")
#                 return JsonResponse({
#                     'error': 'Please provide a response.',
#                     'response': 'I didn\'t receive your answer. Could you please speak again?',
#                     'audio': '',
#                     'audio_duration': 3.0,
#                     'success': False
#                 })
    
#             logger.info(f"Processing valid user input for interview {interview_uuid}: {user_text[:100]}...")
...
#             # Check if interview is already completed
#             if context.get('interview_completed', False):
#                 logger.info(f"Interview {interview_uuid} already completed, sending final response")
#                 return JsonResponse({
#                     'response': 'Thank you! Your interview has been completed.',
#                     'is_final': True,
#                     'success': True,
...
#             current_input_hash = hashlib.md5(user_text.lower().strip().encode()).hexdigest()
#             last_processed = context.get('last_processed_input', '')
            
#             if last_processed == current_input_hash:
#                 logger.warning(f"Duplicate request detected for interview {interview_uuid}")
#                 return JsonResponse({
#                     'error': 'Processing your previous response...',
#                     'success': False
#                 })
...
#             logger.info(f"Question count: {current_count} -> {question_count} for interview {interview_uuid}")
...
#             # FIXED: Simple audio test detection - only for very basic phrases
#             user_lower = user_text.lower().strip()
#             simple_test_phrases = [
#                 'can you hear me', 'hello', 'test', 'testing',
#                 'can you hear me can you hear me'
#             ]
            
#             # Only treat as audio test if it's EXACTLY one of these simple phrases
#             is_simple_audio_test = user_lower in simple_test_phrases and len(user_text.split()) <= 5
            
#             # FIXED: Generate more natural responses
#             if is_simple_audio_test and question_count <= 2:
#                 logger.info(f"Simple audio test detected: '{user_text}' for question {question_count}")
                
#                 if question_count == 1:
#                     ai_response = f"Yes, I can hear you perfectly, {candidate_name}! Your microphone is working great. Let me start with the first question: Can you tell me about yourself and your background? What experiences or skills do you bring to this {job_title} position?"
#                 else:
#                     ai_response = f"I can hear you clearly, {candidate_name}! Let me continue with our interview. Can you tell me more about your technical experience and what interests you about this role?"
                
#                 logger.info(f"Using audio confirmation response for question {question_count}")
            
#             else:
#                 # FIXED: Structured interview flow with natural progression
#                 logger.info(f"Processing substantive response for question {question_count}")
                
#                 interview_questions = {
#                     1: f"Thank you for sharing, {candidate_name}! I'd love to learn more about your technical background. Can you walk me through your experience with programming languages, frameworks, or technologies that are relevant to this {job_title} position?",
                    
#                     2: f"That's great technical experience! Can you tell me about a challenging project you've worked on? I'm interested in understanding the specific problems you encountered and how you approached solving them.",
                    
#                     3: "Excellent problem-solving approach! How do you typically collaborate in team environments? Can you share an example of working with others on a project, especially when there were different opinions or technical approaches?",
                    
#                     4: f"Those are valuable teamwork insights! Looking ahead, where do you see your career going in the next 3-5 years? How does this {job_title} role at {company_name} fit into your professional goals?",
                    
#                     5: "That's a clear career vision! How do you stay current with technology trends and continue learning? What resources or methods do you use for professional development?",
                    
#                     6: f"I appreciate your commitment to learning! Given the specific requirements of this {job_title} role, do you have experience with similar technologies or methodologies? How do you typically approach learning new technical skills when needed?",
                    
#                     7: f"Thank you for those insights! Before we wrap up, I'd love to hear if you have any questions about this {job_title} position, our team, the company culture, or anything else about this opportunity?",
                    
#                     8: f"Thank you for those thoughtful questions, {candidate_name}! I really enjoyed our conversation today. You've shared some great insights about your background and interests. We'll review everything we discussed and get back to you within 2-3 business days with next steps. Thank you for your time!"
#                 }
        
#                 if question_count <= 8:
#                     ai_response = interview_questions.get(question_count, "Can you tell me more about your experience?")
#                     logger.info(f"Using structured question {question_count}")
#                 else:
#                     ai_response = f"Thank you for the comprehensive interview, {candidate_name}! We have everything we need and will be in touch soon."
#                     logger.info(f"Interview extended beyond 8 questions")
...
#             # Keep conversation history manageable (last 20 entries)
#             if len(conversation_history) > 20:
#                 conversation_history = conversation_history[-20:]
                
#             context['conversation_history'] = conversation_history
            
#             # Mark interview as completed if we've reached the end
#             if question_count >= 8:
#                 logger.info(f"Interview completion triggered for {interview_uuid} at question {question_count}")
#                 context['interview_completed'] = True
                
#                 # Generate interview results in background
#                 try:
#                     generate_interview_results(interview, conversation_history)
#                     logger.info(f"Interview results generated for {interview_uuid}")
#                 except Exception as e:
#                     logger.error(f"Failed to generate results for {interview_uuid}: {e}")
            
#             # Save updated context
#             request.session[session_key] = context
#             request.session.modified = True
        
#             # FIXED: ALWAYS generate TTS for responses
#             audio_path = None
#             audio_duration = None
            
#             try:
#                 logger.info(f"Generating TTS for interview {interview_uuid}")
#                 logger.info(f"TTS requested for Monika voice: '{ai_response[:50]}...'")
...
#                     if actual_duration and actual_duration > 0:
#                         audio_duration = actual_duration
#                         logger.info(f"SUCCESS: Monika voice generated: {audio_path}")
#                         logger.info(f"Actual Monika voice duration: {audio_duration:.2f}s")
#                     else:
#                         audio_duration = estimate_audio_duration(ai_response)
#                         logger.info(f"Using estimated duration: {audio_duration:.2f}s")
#                 else:
#                     audio_duration = estimate_audio_duration(ai_response)
#                     logger.warning(f"TTS generation failed - using estimated duration: {audio_duration:.2f}s")
                    
#             except Exception as tts_error:
#                 logger.error(f"TTS generation error for interview {interview_uuid}: {tts_error}")
#                 audio_path = None
#                 try:
#                     from jobapp.tts import estimate_audio_duration
#                     audio_duration = estimate_audio_duration(ai_response)
...
#             # FIXED: Comprehensive response data
#             response_data = {
#                 'response': ai_response,
#                 'audio': audio_path if audio_path else '',
...
#             logger.info(f"Sending response for interview {interview_uuid}:")
#             logger.info(f"  - Question: {question_count}/8")
#             logger.info(f"  - Final: {response_data['is_final']}")
#             logger.info(f"  - Audio: {response_data['has_audio']}")
#             logger.info(f"  - Duration: {audio_duration}s")
            
#             # Clear processing lock
#             if 'last_processed_input' in context:
#                 del context['last_processed_input']
#                 request.session[session_key] = context
#                 request.session.modified = True
            
#             return JsonResponse(response_data)
        
#         # HANDLE GET REQUEST - Initial interview page
#         else:
#             logger.info(f"Serving initial interview page for {interview_uuid}")
            
#             # Generate welcoming first question
#             ai_question = f"Hi {candidate_name}, great to meet you and thank you for joining me today! I'm excited to learn more about you and your background. Could you start by telling me about yourself? What interests you about this {job_title} position at {company_name}, and what experience do you bring to the role?"
            
#             logger.info(f"Initial question generated for interview {interview_uuid}")
...
#                     else:
#                         audio_duration = estimate_audio_duration(ai_question)
#                         logger.info(f"Initial question - using estimated duration: {audio_duration:.2f} seconds")
#                 else:
#                     audio_duration = estimate_audio_duration(ai_question)
#                     logger.info(f"Initial question - no audio generated, using estimated: {audio_duration:.2f} seconds")
...
#         logger.error(f"Interview Error: {error_details}")
        
#         if settings.DEBUG:
#             return HttpResponse(
#                 f'Interview error: {error_details["error_message"]}<br>'
#                 f'<pre>{error_details["traceback"]}</pre>',
#                 status=500
#             )
#         else:
